      ...
      m_messageLabel(new TubeLabel(this))
{
    ...
    m_header->setStyleSheet(QStringLiteral(R"(
        background: %1;
        border-top: 1px solid transparent;
        border-top-left-radius: 4px;
        border-top-right-radius: 4px;
        color: %2;
    )").arg(data.headerBackgroundColor, data.headerTextColor));
    ...
    if (!data.content.isEmpty())
    {
        ...
        m_messageLabel->setStyleSheet(QStringLiteral(R"(
            background: %1;
            border-bottom: 1px solid transparent;
            border-bottom-left-radius: 4px;
            border-bottom-right-radius: 4px;
            color: %2;
        )").arg(data.contentBackgroundColor, data.contentTextColor));
